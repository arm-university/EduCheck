/*H**********************************************************************
* FILENAME : NHD_0216HZ.cpp          DESIGN REF:
*
* DESCRIPTION :
*       NHD_0216HZ class functions definition.
*
* PUBLIC FUNCTIONS :
*
* NOTES :
*       This is the library for the NHD_0216HZ LCD
*
*       Copyright (c) 2020 Arm Limited and affiliates.
*       SPDX-License-Identifier: Apache-2.0
*
* AUTHOR :    Arm Education      START DATE :    January 2021
*
* CHANGES :
*
 * REF NO    DATE                WHO                                       DETAIL
 *           January 2021        Arm Education ()      Original Code
*
*H*/

#include "mbed-os/mbed.h"
#include <cstdarg>
#include "NHD_0216HZ.h"

// Constructor definition for LCD class (NHD_0216HZ) and SPI pin assignment
NHD_0216HZ::NHD_0216HZ(PinName SPI_CS, PinName SPI_MOSI, PinName SPI_SCLK) : _SPI_CS(SPI_CS), _SPI_MOSI(SPI_MOSI), _SPI_SCLK(SPI_SCLK)
{
    _SPI_CS = 0;
    lcd_data0 = "";
    lcd_data1 = "";
    lcd_row = 0;
    lcd_col = 0;
}

// Shift out data via SPI
void NHD_0216HZ::shift_out(int data)
{
    _SPI_CS = 0;
    for (int i = 0; i < 8; i++)
    {
        _SPI_MOSI = (data & (0x80 >> i));
        _SPI_SCLK = 0;
        wait_us(1);
        _SPI_SCLK = 1;
    }
    _SPI_MOSI = 0;
    _SPI_CS = 1;
}

// Initialize and set up LCD
void NHD_0216HZ::init_lcd(void)
{
    wait_us(40000);
    shift_out(0x30); //function set 8-bit
    wait_us(37);
    write_cmd(0x20); //function set
    wait_us(37);
    write_cmd(0x20); //function set
    wait_us(37);
    write_cmd(0x0C); //display ON/OFF
    wait_us(37);
    write_cmd(0x01); //display clear
    wait_us(1520);
    write_cmd(0x06); //entry-mode set
    wait_us(37);
    write_cmd(0x28); //function set
    wait_us(37);
    set_cursor(0, 0);
}

// Transfer out data by 4-bits using shift_out function
void NHD_0216HZ::write_4bit(int data, int mode)
{
    int hi_n;
    int lo_n;

    hi_n = (data & 0xF0);
    lo_n = ((data << 4) & 0xF0);

    shift_out(hi_n | ENABLE | mode);
    wait_us(1);
    shift_out(hi_n & ~ENABLE);
    shift_out(lo_n | ENABLE | mode);
    wait_us(1);
    shift_out(lo_n & ~ENABLE);
}

// Write command using write_4bit function in COMMAND mode
void NHD_0216HZ::write_cmd(int cmd)
{
    write_4bit(cmd, COMMAND_MODE);
}

// Write data using write_4bit function in DATA mode
void NHD_0216HZ::write_data(char c)
{
    write_4bit(c, DATA_MODE);
}

// Print characters
void NHD_0216HZ::printf(const char *format, ...)
{
    va_list v;
    char buffer[16];
    va_start(v, format);
    vsprintf(buffer, format, v);

    char *b = buffer;
    for (int i = 0; i < 16 && *b; i++)
    {
        write_data(*b++);
    }

    va_end(v);

    if (lcd_row == 0)
    {
        lcd_data0 = std::string(format);
    }

    else
    {
        lcd_data1 = std::string(format);
    }
}

// Set cursor
void NHD_0216HZ::set_cursor(int column, int row)
{
    int addr;

    addr = (row * LINE_LENGTH) + column;
    addr |= TOT_LENGTH;
    write_cmd(addr);

    lcd_row = row;
    lcd_col = column;
}

// Clear LCD
void NHD_0216HZ::clr_lcd(void)
{
    write_cmd(0x01); //display clear
    wait_us(1520);
}

// Return data displayed on LCD (used in Educheck testing)
string NHD_0216HZ::check_lcd_data(int row)
{

    lcd_row = row;
    if (lcd_row == 0)
    {
        return lcd_data0;
    }

    else
    {
        return lcd_data1;
    }
}
